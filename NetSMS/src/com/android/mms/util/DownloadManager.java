...
public class DownloadManager {
    ...
    private final OnSharedPreferenceChangeListener mPreferencesChangeListener =
        ...
        public void onSharedPreferenceChanged(SharedPreferences prefs, String key) {
            if ("pref_key_mms_auto_retrieval".equals(key)
                    || "pref_key_mms_retrieval_during_roaming".equals(key)) {
                if (LOCAL_LOGV) {
  ...
  //              state.setFromNotifierBundle(intent.getExtras());
    ...
    static boolean getAutoDownloadState(SharedPreferences prefs, boolean roaming) {
        boolean autoDownload = prefs.getBoolean(
        		"pref_key_mms_auto_retrieval", true);
        ...
        if (autoDownload) {
            boolean alwaysAuto = prefs.getBoolean(
            		"pref_key_mms_retrieval_during_roaming", false);
        ...
        try {
            ...
            if ((nInd.getExpiry() < System.currentTimeMillis()/1000L)
                ...
                mHandler.post(new Runnable() {
                    public void run() {
                        Toast.makeText(mContext,"Message out update or not have",
